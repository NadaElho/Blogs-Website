...
const getBlogDetails = async(req, res)=>{
    let blogs = await Blog.findById(req.params.id).populate("category").populate("userId")
    res.status(200).send(blogs)
}
...
const addNewBlog = async(req, res)=>{
    ...
    await Blog.create({title, description, userId, image: imageUrl, date: datetime, content, category})
...
const editBlog = async (req, res) => {
    try {
        ...
        if (imageUrl) {
            await Blog.updateOne({ _id: req.params.id }, { title, description, image: imageUrl, content, category });
        } else {
            await Blog.updateOne({ _id: req.params.id }, { title, description, content, category });
        }
